...
function [g,h] = exemplo_nolinear_constrains_24(x)
% ---
...
g(2) = msocneg - mresneg;   % restrição do momento no engast  e h.2 %
g(3) = msocposy - mresposy; % restrição do momento em y h.3
...
h(1) = x2 - ((0.85*fcd*bw)/fyd)*x5; % restrições As - 0.85fcdbwx/fyd
h(2) = x3 - ((0.85*fcd*bw)/fyd)*x6; % restrições As - 0.85fcdbwx/fyd
h(3) = x4 - ((0.85*fcd*bw)/fyd)*x7; % restrições As - 0.85fcdbwx/fyd
% 
